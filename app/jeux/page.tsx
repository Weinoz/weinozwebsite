...
import { games, GameStatus, GamePlatform } from '@/data/games';
import { Star, Clock, Gamepad2, Trophy, Play, X, BookMarked, SortAsc } from 'lucide-react';
...
export default function JeuxPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [activeStatus, activePlatform, sortBy]);

  const totalHours = games.reduce((acc, g) => acc + (g.hours ?? 0), 0);
  const completedCount = games.filter((g) => g.status === 'terminé').length;
  const avgRating = (
    games.filter((g) => g.rating !== undefined).reduce((acc, g) => acc + (g.rating ?? 0), 0) /
    games.filter((g) => g.rating !== undefined).length
  ).toFixed(1);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-16">
      ...
      <div className="mb-10">
        ...
        <p className="text-purple-300/50">{games.length} jeux répertoriés</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
        {[
          { label: 'Jeux totaux', value: games.length, icon: <Gamepad2 className="w-5 h-5" /> },
          ...
          { label: 'Note moyenne', value: avgRating + '/10', icon: <Star className="w-5 h-5" /> },
      ...
      <div className="flex flex-col gap-4 mb-8">
        ...
        <div className="flex flex-wrap gap-2">
          {statusFilters.map(({ value, label, icon }) => {
            ...
            const count = value === 'all' ? games.length : games.filter((g) => g.status === value).length;
            return (
              <button
                key={value}
